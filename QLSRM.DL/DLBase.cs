...
namespace QLSRM.DL
{
    public class DLBase
    {
                ...
                using (var cmd = new MySqlCommand())
                {
                    ...
                    if (item != null)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(storedName))
                        {
                            ...
                            if (success)
                            {
                                if (cmd != null && cmd.Parameters?.Count > 0 && cmd.Parameters.Contains("@Id") && !string.IsNullOrWhiteSpace(cmd?.Parameters?["@Id"]?.Value?.ToString()))
                                {
                                    switch (value)
                                    {
                                        case EditMode.Add:
                                            var idProp = item.GetType().GetProperty("Id");
                                            if (idProp != null && idProp.GetValue(item) is long)
                                            {
                                                idProp.SetValue(item, long.Parse(cmd?.Parameters?["@Id"]?.Value?.ToString()));
                                            }
                                            break;
                                    }
                                }
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public List<T> SelectAll<T>(object param)
        {
